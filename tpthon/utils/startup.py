...
from tpthon import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
async def setup_bot():
    """
    To set up bot for tpthon
    """
    try:
        await tipthon.connect()
        config = await tipthon(functions.help.GetConfigRequest())
        for option in config.dc_options:
            if option.ip_address == tipthon.session.server_address:
                if tipthon.session.dc_id != option.id:
                    LOGS.warning(
                        f"ايـدي DC ثـابت فـي الجلسـة مـن {tipthon.session.dc_id}"
                        f" الـى {option.id}"
                    )
                tipthon.session.set_dc(option.id, option.ip_address, option.port)
                tipthon.session.save()
                break
        bot_details = await tipthon.tgbot.get_me()
        Config.TG_BOT_USERNAME = f"@{bot_details.username}"
        # await tipthon.start(bot_token=Config.TG_BOT_USERNAME)
        tipthon.me = await tipthon.get_me()
        tipthon.uid = tipthon.tgbot.uid = utils.get_peer_id(tipthon.me)
        if Config.OWNER_ID == 0:
            Config.OWNER_ID = utils.get_peer_id(tipthon.me)
...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.ZEDUBLOGO = await tipthon.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/198fe72062f994c6e6e02.jpg",
                caption="**•⎆┊تـم بـدء تشغـيل سـورس تيبثـــون الخاص بك .. بنجاح 🧸♥️**",
                buttons=[(Button.url("𝗧𝗶𝗽𝘁𝗵𝗼𝗻 𝗮𝗟 𝗔𝗿𝗮𝗯", "https://t.me/E9N99"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            await tipthon.check_testcases()
            message = await tipthon.get_messages(msg_details[0], ids=msg_details[1])
            text = message.text + "\n\n**•⎆┊تـم اعـادة تشغيـل السـورس بنجــاح 🧸♥️**"
            await tipthon.edit_message(msg_details[0], msg_details[1], text)
            if gvarstatus("restartupdate") is not None:
                await tipthon.send_message(
                    msg_details[0],
                    f"{cmdhr}بنك",
...
async def mybot():
    ...
    zilbot = await tipthon.tgbot.get_me()
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", "TipThon")
            ...
            await bot.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await bot.send_file("@BotFather", "tpthon/tiplogo/logotp.jpg")
            await asyncio.sleep(3)
            await bot.send_message("@BotFather", "/setabouttext")
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", f"- بـوت تيبثـــون المسـاعـد ♥️🦾 الخـاص بـ  {bot.me.first_name} ")
            await asyncio.sleep(3)
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"tpthon/{folder}/*.py"
        plugin_path = f"tpthon/{folder}"
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await tipthon(JoinChannelRequest("@E9N99"))
    except BaseException:
        pass
    try:
        await tipthon(JoinChannelRequest("@a_aaaf"))
    except BaseException:
        pass
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await tipthon.get_entity(BOTLOG_CHATID)
        ...
        photozed = await tipthon.upload_file(file="tepthon/malath/Zpic.jpg")
        _, groupid = await create_supergroup(
            "كـروب السجـل تيبثــون", tipthon, Config.TG_BOT_USERNAME, descript, photozed
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        try:
            entity = await tipthon.get_entity(PM_LOGGER_GROUP_ID)
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
                    LOGS.info(
        ...
        photozed = await tipthon.upload_file(file="tepthon/malath/Apic.jpg")
        _, groupid = await create_supergroup(
            "كـروب التخـزين", tipthon, Config.TG_BOT_USERNAME, descript, photozed
        )
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if response.code != 200:
        LOGS.error(errtext)
        return await tipthon.tgbot.send_message(BOTLOG_CHATID, errtext)
    await runcmd(gcmd)
    if not os.path.exists(cfolder):
        LOGS.error(
            "There was a problem in cloning the external repo. please recheck external repo link"
        )
        return await tipthon.tgbot.send_message(
            BOTLOG_CHATID,
            "There was a problem in cloning the external repo. please recheck external repo link",
        )
    ...
    await load_plugins(folder="tpthon", extfolder=cfolder)
