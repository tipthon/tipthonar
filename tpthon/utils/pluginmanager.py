...
from tpthon import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        checkplugins(path)
        name = "tpthon.plugins.{}".format(shortname)
        ...
        if plugin_path is None:
            path = Path(f"tpthon/plugins/{shortname}.py")
            name = f"tpthon.plugins.{shortname}"
        else:
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"tpthon.plugins.{shortname}"] = mod
        LOGS.info(f"تـم تثبيت ملـف {shortname}")
...
def remove_plugin(shortname):
        ...
        for cmdname in cmd:
            if cmdname in LOADED_CMDS:
                for i in LOADED_CMDS[cmdname]:
                    tipthon.remove_event_handler(i)
    ...
    try:
        name = f"tpthon.plugins.{shortname}"
        for i in reversed(range(len(tipthon._event_builders))):
            ev, cb = tipthon._event_builders[i]
            if cb.__module__ == name:
                del tipthon._event_builders[i]
