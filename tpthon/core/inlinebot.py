...
ZEDLOGO = "https://telegra.ph/file/1d735babbe8b94cb588ea.jpg"
MALATH_PIC = "https://telegra.ph/file/1d735babbe8b94cb588ea.jpg"
...
def main_menu():
    malathy = f"**🖥┊لـوحـة اوامـر 𝗧𝗶𝗽𝘁𝗵𝗼𝗻 𝗮𝗟 𝗔𝗿𝗮𝗯 الشفافـه**\
        \n**🧑🏻‍💻┊المستخـدم ↶** {mention}"
    if Config.OLDZED:
        buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**TpThon"):
            ...
            builder = event.builder
            troll = os.path.join("./tpthon", "troll.txt")
            try:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./tpthon", "secrets.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./tpthon", "hide.txt")
            try:
            ...
            result = builder.article(
                title="© TipThon Help",
                description="Help menu for TipThon",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        text, msg_entities = await event.client._parse_message_text(
            "𝗗𝗲𝗽𝗹𝗼𝘆 𝘆𝗼𝘂𝗿 𝗼𝘄𝗻 𝗧𝗶𝗽𝘁𝗵𝗼𝗻 𝗮𝗟 𝗔𝗿𝗮𝗯.", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="𝗧𝗶𝗽𝘁𝗵𝗼𝗻 𝗮𝗟 𝗔𝗿𝗮𝗯",
...
@tipthon.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@tipthon.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@tipthon.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@tipthon.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
