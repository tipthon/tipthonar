...
from tpthon import Config, tipthon
    ...
    if chat.id != Config.OWNER_ID:
        ...
        if customstrmsg is not None:
            ...
            start_msg = f"**❈╎مـرحباً بـك عزيـزي  {mention} **\
                        ...
                        \n\n**❈╎البـوت خـاص بسـورس :** [𝗧𝗶𝗽𝘁𝗵𝗼𝗻](https://t.me/E9N99)"
        buttons = [
            (
...
@tipthon.bot_cmd(incoming=True, func=lambda e: e.is_private)
...
@tipthon.bot_cmd(pattern="^/uinfo$", from_users=Config.OWNER_ID)
async def bot_start(event):
    reply_to = await reply_id(event)
    if not reply_to:
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await tipthon.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await tipthon.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await tipthon.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await tipthon.tgbot.get_entity(BOTLOG_CHATID)
            await tipthon.tgbot.send_message(
                Config.OWNER_ID,
...
@tipthon.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
async def bot_pm_ban_cb(c_q: CallbackQuery):
    user_id = int(c_q.pattern_match.group(1))
    try:
        user = await tipthon.get_entity(user_id)
    except Exception as e:
...
@tipthon.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@tipthon.bot_cmd(incoming=True, func=lambda e: e.is_private)
@tipthon.bot_cmd(edited=True, func=lambda e: e.is_private)
