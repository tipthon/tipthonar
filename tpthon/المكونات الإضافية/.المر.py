...
TEPPIC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/1d735babbe8b94cb588ea.jpg"
TPIC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/1d735babbe8b94cb588ea.jpg"
TG_BOT = Config.TG_BOT_USERNAME
TM = time.strftime("%I:%M")


@tipthon.tgbot.on(events.InlineQuery)
@check_owner
async def tip_handler(event):
    builder = event.builder
    result = None
    query = event.text
    await tipthon.get_me()
    if query.startswith("الاوامر") and event.query.user_id == tipthon.uid:
        Malath = f"**🖥┊لـوحـة اوامـر 𝗧𝗶𝗽𝘁𝗵𝗼𝗻 الشفـافـه **\n**🧑🏻‍💻┊المستخـدم ↶** {mention} \n\n**•❐• قـائمـه الاوامـر الـعـامـه :**\n**•❶• اوامــر الـبحـث والـتحميـل **\n**•❷• اوامــر الـبـوت **\n**•❸• اوامــر الـوقـتـي **\n**•❹• اوامــر الـكــروب¹ **\n**•❺• اوامــر الـكــروب² **\n**•❻• اوامــر الـحسـاب **\n**•❼• اوامــر الميـديـا والـصيــغ **\n**•❽• اوامــر الـفــارات **\n**•❾• اوامــر الخـدمــات **\n**•❿• اوامــر الـتســليــه والالـعـاب **\n\n**•❐• او استخـدم الامـر** `.الأوامر` **لعـرض الاوامـر مع الوصـف**"
        ...
        if TEPPIC and TEPPIC.endswith((".jpg", ".png")):
            result = builder.photo(TEPPIC, text=Malath, buttons=buttons, link_preview=True)
        elif TEPPIC and TEPPIC.endswith((".mp4")):
            result = builder.document(TEPPIC,title="tipthon", text=Malath ,buttons=buttons, link_preview=True)
        else:
            result = builder.article(title="tipthon",text=Malath,buttons=buttons,link_preview=True)
        await event.answer([result] if result else None)
@tipthon.ar_cmd(pattern="الاوامر(?: |$)(.*)")
async def repotipthon(event):
    if event.fwd_from:
        return
    TG_BOT = Config.TG_BOT_USERNAME
...
async def tip_handler(tipthon):
    ...
    zilzal = [[Button.inline("رجــوع", data="ahmed")]]
    await tipthon.edit(text, buttons=zilzal)
@tipthon.tgbot.on(CallbackQuery(data=re.compile(rb"ahmed9")))
@check_owner
async def tip_handler(tipthon):
    text = "**✾╎قائـمه اوامـر الـخدمـات :**\n\n**⎞𝟏⎝** `.التخصيص`\n\n**⎞𝟐⎝** `.الترحيب`\n\n**⎞𝟑⎝** `.الردود`\n\n**⎞𝟒⎝** `.الاذاعه`\n\n**⎞𝟓⎝** `.النشر`\n\n**⎞𝟔⎝** `.الكاشف`\n\n**⎞𝟕⎝** `.ستوريات`\n\n**⎞𝟖⎝** `.التحشيش`\n\n**⎞𝟗⎝** `.المساعد`\n\n**⎞𝟏??⎝** `.الطقس`"
    zilzal = [[Button.inline("رجــوع", data="ahmed")]]
    await tipthon.edit(text, buttons=zilzal)
@tipthon.tgbot.on(CallbackQuery(data=re.compile(rb"ahmad10")))
@check_owner
async def tip_handler(tipthon):
...
@check_owner
async def tip_handler(tipthon):
    builder = tipthon.builder
    result = None
    query = tipthon.text
    await bot.get_me()
    if query.startswith("تيبثون") and tipthon.query.user_id == bot.uid:
        channels = f"**•❐• مـرحبــاً عـزيـزي  {mention} **\n**•❐• اليـك مجمـوعــة قنـوات تيبثـــون ↵ 𝗧𝗶𝗽𝘁𝗵𝗼𝗻 𝗮𝗟 𝗔𝗿𝗮𝗯 ♥️🦾**\n\n**•❐• استـخـدم الازرار بالاسفــل↓**"
        buttons = [[Button.url("قنـاة السـورس", "https://t.me/E9N99"),],[Button.url("التحـديثـات", "https://t.me/a_aaaf"), Button.url("الفـارات", "https://t.me/aa_aaf"),],[Button.url("الشـروحـات¹", "https://t.me/a_aaaf"),],[Button.url("الكلايش", "https://t.me/clccca"),],[Button.url("مطـور السـورس", "https://t.me/M_H_N"),]]
        if TEPPIC and TEPPIC.endswith((".jpg", ".png", "gif", "mp4")):
            result = builder.photo(TEPPIC, text=channels, buttons=buttons, link_preview=False)
        elif TEPPIC:
            result = builder.document(TEPPIC,title="tipthon",text=channels,buttons=buttons,link_preview=False)
        else:
            result = builder.article(title="tipthon",text=channels,buttons=buttons,link_preview=False)
        await tipthon.answer([result] if result else None)
@tipthon.ar_cmd(pattern="تيبثون")
async def repotipthon(tipthon):
    if tipthon.fwd_from:
        return
    TG_BOT = Config.TG_BOT_USERNAME
    if tipthon.reply_to_msg_id:
        await tipthon.get_reply_message()
    response = await bot.inline_query(TG_BOT, "تيبثون")
    await response[0].click(tipthon.chat_id)
    await tipthon.delete()
...
async def tip_handler(event):
    ...
    query = event.text
    await tipthon.get_me()
    if query.startswith("سورس") and event.query.user_id == tipthon.uid:
        Zelzal = f"**•◈• اصــدار الســورس ⤽ 7.7**  \n**•◈• المستخــدم ⤽**  {mention}  \n**•◈• وقــت التشغيــل ⤽  {TM}  **\n**•◈• البــوت المســاعـد ⤽  {TG_BOT} **\n**•◈• قنــاة الســورس ⤽  @E9N99 **"
        buttons = [[Button.url("قنـاة الســورس", "https://t.me/E9N99"), Button.url("مطـور الســورس", "https://t.me/M_H_N")]]
        if TEPPIC and TEPPIC.endswith((".jpg", ".png", ".gif", ".mp4")):
            result = builder.photo(TEPPIC, text=Zelzal, buttons=buttons, link_preview=True)
        elif TEPPIC:
            result = builder.document(TEPPIC,title="tipthon",text=Zelzal,buttons=buttons,link_preview=True)
        else:
            result = builder.article(title="tipthon",text=Zelzal,buttons=buttons,link_preview=True)
        await event.answer([result] if result else None)
@tipthon.ar_cmd(pattern="سورس")
async def repotipthon(event):
    if event.fwd_from:
        return
    TG_BOT = Config.TG_BOT_USERNAME
    if event.reply_to_msg_id:
        await event.get_reply_message()
    response = await tipthon.inline_query(TG_BOT, "سورس")
    await response[0].click(event.chat_id)
    await event.delete()
