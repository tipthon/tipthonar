...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "TEP - THON"


@tipthon.ar_cmd(pattern=r"تهكير$")
async def _(event):
    if event.fwd_from:
        return
    if event.reply_to_msg_id:
        ...
        if idd == 925972505 or idd == 1895219306 or idd == 2028523456 or idd == 1488114134:
            await edit_or_reply(
            ...
            animation_chars = [
                "**⌔: جاري الاتصال بخوادم تيبثــون المتخصصه بالـتهكير**",
...
@tipthon.ar_cmd(pattern=f"thack$")
async def _(event):
    if event.fwd_from:
        return
...
@tipthon.ar_cmd(pattern=f"wahack$")
async def _(event):
    if event.fwd_from:
        return
    animation_interval = 2
