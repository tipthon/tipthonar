...
async def cmdlist():
    outstr = "**Total list of Commands in your TpThon are :**\n\n"
    category = ["الادمن", "البوت", "الترفيه", "الخدمات", "الادوات", "العروض", "البحث"]
    if OLDZED:
        category.append("آخرى")
    for cat in category:
...
@tipthon.ar_cmd(
    ...
    info={
        "header": "To get guide for TpThon.",
...
async def _(event):
    ...
    await edit_or_reply(
        event, outstr, aslink=True, linktext="Total Commands of TpThon are :"
    )
...
async def _(event):
    ...
    if found := [i for i in sorted(list(CMD_INFO)) if cmd in i]:
        ...
        out = f"I can't find any such command `{cmd}` in TpThon"
