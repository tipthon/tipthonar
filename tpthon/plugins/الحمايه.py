...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if custompmpermit is not None:
        ...
        USER_BOT_NO_WARN = f"""𓆩𝗧𝗶𝗽𝘁𝗵𝗼𝗻 𝗮𝗟 𝗔𝗿𝗮𝗯 - 𝑷𝑴 𝑺𝑬𝑪𝑼𝑹𝑰𝑻𝒀𓆪
◐━─━─━─━─𝗧𝗶𝗽𝘁𝗵𝗼𝗻─━─━─━─━◐
...
@tipthon.ar_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@tipthon.ar_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@tipthon.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
@tipthon.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⤶ عـذراً سيـدي ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🤷🏻‍♂"
        return await event.answer(text, cache_time=0, alert=True)
...
@tipthon.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⤶ عـذراً سيـدي ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🤷🏻‍♂"
        return await event.answer(text, cache_time=0, alert=True)
...
@tipthon.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⤶ عـذراً سيـدي ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🤷🏻‍♂"
        return await event.answer(text, cache_time=0, alert=True)
...
@tipthon.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⤶ عـذراً سيـدي ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🤷🏻‍♂"
        return await event.answer(text, cache_time=0, alert=True)
...
@tipthon.ar_cmd(pattern="بلوك(?:\s|$)([\s\S]*)")
...
@tipthon.ar_cmd(pattern="الغاء بلوك(?:\s|$)([\s\S]*)")
...
@tipthon.ar_cmd(pattern="المقبولين$")
