...
@tipthon.ar_cmd(pattern="دائري ?((-)?s)?$")
...
@tipthon.ar_cmd(pattern="ملصق متحرك(?: |$)(.*)")
...
@tipthon.ar_cmd(pattern="لمتحركه(?: |$)((-)?(r|l|u|d|s|i)?)$")
...
@tipthon.ar_cmd(pattern="متحرك ?([0-9.]+)?$")
async def _(event):
    reply = await event.get_reply_message()
