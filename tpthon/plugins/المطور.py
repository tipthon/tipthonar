...
TDEV = gvarstatus("sudoenable") or "true"

TipthonDV_cmd = (
    "𓆩 [𝗧𝗶𝗽𝘁𝗵𝗼𝗻 𝗮𝗟 𝗔𝗿𝗮𝗯 𝗖𝗼𝗻𝗳𝗶𝗴 - اوامــر المطـور المســاعد](t.me/E9N99) 𓆪\n\n"
...
async def chat_blacklist(event):
    ...
    if input_str == "تفعيل":
        ...
        if len(sudousers) != 0:
            text += (
                "**✾╎يتم الان اعـادة تشغيـل بـوت تيبثــون انتظـر 2-1 دقيقـه ▬▭ ...**"
            )
...
async def add_sudo_user(event):
    ...
    output += "**✾╎يتم الان اعـادة تشغيـل بـوت تيبثــون انتظـر 2-1 دقيقـه ▬▭ ...**"
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "عام":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your TipThon__.\n"
                )
            else:
        ...
        for cmd in input_str:
            if cmd not in CMD_INFO:
                errors += f"`{cmd}` __There is no such command in your TipThon__.\n"
    ...
    output = (
        result + "**✾╎يتم الان اعـادة تشغيـل بـوت تيبثــون انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
