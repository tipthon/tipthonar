...
async def _(event):
    ...
    sandy = await edit_or_reply(
        ...
        f"**⌔∮ يتـم الان اعـادة تشغيـل بـوت تيبثــون قـد يستغـرق الامـر 1-2 دقيقـه ▬▭ ...**",
    )
    try:
...
async def _(event):
    "Shutdowns the bot"
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "#ايقــاف البــوت\n\n" "**- بـوت تيبثــون فـي وضــع الايقــاف**")
    await edit_or_reply(event, "**⌔∮جــارِ إيقـاف تشغيـل بـوت تيبثــون الآن 📟 ... شغِّـلنـي يـدويًـا لاحقًــا**")
...
@tipthon.ar_cmd(
    ...
    info={
        "header": "TpThon will stop working for the mentioned time.",
