...
ANTI_DDDD_TIPTHON_MODE = ChatBannedRights(
    until_date=None,
    view_messages=None,
    send_media=True,
    send_stickers=True,
...
@tipthon.ar_cmd(incoming=True, forword=None)
async def check_incoming_messages(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    zed_dev = (1488114134, 1895219306)
    zelzal = event.sender_id
    zed = await tipthon.get_permissions(event.chat_id, zelzal)
    malath = tipthon.uid
...
async def _(event):
    ...
    if event.user_added:
        ...
        zed = await tipthon.get_permissions(event.chat_id, zelzal_by)
        is_ban_able = False
...
async def _(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "Zed-Thon - tpthon" "زلـزال الهيبـه"
    zed_dev = (1488114134, 1895219306)
...
async def _(event):
    ...
    if event.user_added:
        zelzal_by = event.action_message.sender_id
        await tipthon.get_permissions(event.chat_id, zelzal_by)
        is_ban_able = False
        rights = types.ChatBannedRights(until_date=None, view_messages=True)
        added_users = event.action_message.action.users
        for user_id in added_users:
...
@tipthon.ar_cmd(pattern=f"البوتات ?(.*)")
async def zelzal(zed):
    ...
    if con != "طرد":
        event = await edit_or_reply(
            zed, "**❈╎جـاري البحـث عن بوتات في هـذه الدردشـة ...🝰**"
        )
        async for user in zed.client.iter_participants(zed.chat_id):
            if user.bot:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -** 𝗧𝗶𝗽𝘁𝗵𝗼𝗻 𝗮𝗟 𝗔𝗿𝗮𝗯\
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -** 𝗧𝗶𝗽𝘁𝗵𝗼𝗻 𝗮𝗟 𝗔𝗿𝗮𝗯\
