...
from tpthon import StartTime, tipthon, zedversion
...
@tipthon.ar_cmd(pattern=f"{STATS}$")
async def amireallyalive(event):
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "** بـوت  تيبثــون 𝗧𝗶𝗽𝘁𝗵𝗼𝗻  يعمـل .. بنجـاح ☑️ 𓆩 **"
...
**{Z_EMOJI} إصـدار تيبثــون :** `{zdver}`
**{Z_EMOJI} إصـدار البـايثون :** `{pyver}`
**{Z_EMOJI} الوقـت :** `{uptime}`
**{Z_EMOJI} المسـتخدم:** {mention}
**{Z_EMOJI} قنـاة السـورس :** [اضغـط هنـا](https://t.me/E9N99)"""
...
async def amireallyialive(event):
    ...
    reply_to_id = await reply_id(event)
    Z_EMOJI = gvarstatus("ALIVE_EMOJI") or "✥┊"
    zed_caption = "** بـوت  تيبثــون 𝗧𝗶𝗽𝘁𝗵𝗼𝗻  يعمـل .. بنجـاح ☑️ 𓆩 **\n"
...
@tipthon.tgbot.on(CallbackQuery(data=re.compile(b"stats")))
