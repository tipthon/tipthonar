...
@tipthon.ar_cmd(pattern=f"{ADMZ}(?:\s|$)([\s\S]*)")
...
@tipthon.ar_cmd(pattern="رفع مالك(?:\s|$)([\s\S]*)")
async def promote(event):
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
...
@tipthon.ar_cmd(pattern="اخفاء(?:\s|$)([\s\S]*)")
async def promote(event):
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
...
@tipthon.ar_cmd(pattern=f"{UNADMZ}(?:\s|$)([\s\S]*)")
...
@tipthon.ar_cmd(pattern=f"{BANN}(?:\s|$)([\s\S]*)")
async def _ban_person(event):
    ...
    if user.id == 925972505 or user.id == 1895219306 or user.id == 1488114134:
        return await edit_delete(
            event, "**╮ ❐ دي لا يمڪنني حظـر احـد مطـورين السـورس  ❏╰**"
        )
...
@tipthon.ar_cmd(pattern=f"{UNBANN}(?:\s|$)([\s\S]*)")
...
@tipthon.ar_cmd(pattern=f"{UNMUTE}(?:\s|$)([\s\S]*)")
...
@tipthon.ar_cmd(pattern=f"{KICK}(?:\s|$)([\s\S]*)")
